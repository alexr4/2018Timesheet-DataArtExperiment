...
void setup() {
  ...
  for (int i=0; i<3; i++) {
    ...
    float x = cos(theta + thetaStart) * radius + ox;
    float y = sin(theta + thetaStart) * radius + oy;
  ...
  PVector B = vertList.get(0+1);
  PVector C = vertList.get(0+2);
  //subdivOnTwo(A,B,C,it);
  subdivGravityCenter(A, B, C, it);
  //subdivGravityCenterInside(A, B, C, it, 0.1, it);
  //subdivGravityCenterOffset(A,B,C,it, new PVector(0, 1), 400.0, it);
  //subdivMediatrice(A, B, C, it);
  //subdivGravityCenterMediatrice(A, B, C, it);
  //subdivGravityCenterMediatrice(A, B, C, it, 0.5, it);
  ...
  //Offseting
  /*
  PVector center = new PVector(buffer.width/2, buffer.height/2);
  float deviation = 5.0;//radius * 0.01;
  for (int i=3; i<vertList.size(); i+=3) {
    PVector A_ = vertList.get(i);
    PVector B_ = vertList.get(i+1);
    PVector C_ = vertList.get(i+2);

    PVector AB = PVector.sub(A_, B_);
    PVector AC = PVector.sub(A_, C_);
    PVector CenterA = PVector.sub(center, A_);

    float distA = 1.0 - PVector.dist(A_, center)/radius;
    AB.normalize();
    AC.normalize();
    CenterA.normalize();

    AB.mult(deviation * distA);
    AC.mult(deviation * distA);
    CenterA.mult(deviation * distA);
...
void draw() {
  ...
  //quadtree.debug(g);
  noFill();
  //computeBuffer(g, vertList);
  //computeBuffer(g, smoothVertList);
  imageMode(CENTER);
  float x = width/2;
  float y = height/2;
  float w = width;
...
void subdivGravityCenterMediatrice(PVector A, PVector B, PVector C, int iteration) {
  if (iteration <= 0) {
    vertList.add(A);
    vertList.add(B);
    vertList.add(C);
    ...
    println("\tSubdiv "+iteration+" done");
    iteration --;
    subdivGravityCenterMediatrice(A, AB, gravity, iteration);
    subdivGravityCenterMediatrice(AB, B, gravity, iteration);
    subdivGravityCenterMediatrice(A, gravity, AC, iteration);
    subdivGravityCenterMediatrice(AC, gravity, C, iteration);
    //subdivGravityCenterMediatrice(gravity, B, BC, iteration);
    //subdivGravityCenterMediatrice(gravity, BC, C, iteration);
  }
}
