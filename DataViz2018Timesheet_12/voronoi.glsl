...
uniform sampler2D image; //gradient1.png
...
void main(){
  ...
  VoroStruct voronoi = voronoiDistance(st, colsrows, 3.5, 0.25, 0.65);
  float border = getBorder(voronoi, 0.025, 0.05, 0.001, 0.01, 0.01, 0.015);
  vec2 index = voronoi.indices;

  vec2 texel = 1.0 / colsrows;
  vec2 toCenter = vec2(0.5) - index;
  float angle   = atan(toCenter.y, toCenter.x) + PI;
  float dist = length(vec2(0.5) - index) / (sqrt(2.0) * 0.5);
  ...
  float isOutsideMonth = 1.0 - step(31.0, offsetedIndex);
  ...
  float stepper = (isRight * isLeft * isTop * isBottom);//1.0 - step(0.5, dist);//(isRight * isLeft * isTop * isBottom);//
  vec3 color = vec3(border) * stepper * isOutsideMonth + vec3(index, 0.0) * 0.0;
  float colorIndex = random(index);
//  float indexDay = i / (colsrows.y * colsrows.x);
  vec3 rgbIndex = vec3(
    0.0,
    0.0,
    random(index.yx)
  ...
  float indexuv = floor(colorIndex * 4.0) / 4.0;
  vec3 fcolor = texture2D(image, vec2(indexuv + 0.01, 0.5)).rgb;

  float grey = 239.0/255.0;
  vec3 background = grey - color;

  gl_FragColor = vec4(background, 1.0) * 1.0 + vec4(color * fcolor, 1.0);
}
