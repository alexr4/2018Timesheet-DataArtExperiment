class Veneation {
  ...
  Veneation(int nbOfNodes, int nbOfFractureNode) {
   ...
   Veneation(int nbOfNodes, int nbOfFractureNode, PGraphics ctx) {
    this.nbOfNodes = nbOfNodes;
    this.nbOfFractureNode = nbOfFractureNode;
  ...
  void initNodes() {
    ...
    float margin = 10;
    float hyp = sqrt(ctx.width * ctx.width + ctx.height * ctx.height);
    maxRadius = ctx.width * 0.45;
    ...
    for (int i=0; i<this.nbOfNodes; i++) {
       ...
       */
      float x = random(margin, ctx.width-margin);
      float y = random(margin, ctx.height-margin);
  ...
  void initRoot() {
    ...
    float x = cos(theta) * radius + ctx.width * 0.5;
    float y = sin(theta) * radius + ctx.height * 0.5;
  ...
  void debugPoint(PGraphics b, ArrayList<Node> list, float size) {
    b.noStroke();
    PVector center = new PVector(ctx.width * 0.5, ctx.height*0.5);
    for (Node node : list) {
