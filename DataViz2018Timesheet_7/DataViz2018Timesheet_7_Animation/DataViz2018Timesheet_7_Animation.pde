...
void settings() {
  ...
  float scale = 0.5;
  size(round(1080 * scale), round(1920 * scale), P2D);
  smooth(8);
}

void setup() {
  pt = new PerfTracker(this, 100);
  float res = 1080.0 / 1920.0;
  int h = 1080;
  int w = round(h * res);
  ...
  ArrayList<Float> dataDay = getDataPerDay(4);
  printArray(dataDay);

  sps = new Veneation(round(dataDay.get(1)) * 12, round(dataDay.get(1)) * 12, dataBuffer);
...
void draw() {
  ...
  if (sps.nodeList.size() <= 0) {
    ...
    dataBuffer.save("Friday/SpaceColonization_"+nf(frameCount, 4)+".png");
  }
