class Veneation {
  ...
  void initRoot() {
    branchList.clear();
    //root
    float radius = random(0);//maxRadius;//
  ...
  void grow() { 
    ...
    for (Node node : nodeList) {
      ...
      minDist = map(nd, 0.0, 1.0, oMaxDist, oMinDist);
      minDist += noise * 2.0;
  ...
  void debugBranch(PGraphics b, ArrayList<Branch> list, color rgb, float size) {
    b.noFill();
    for (Branch branch : list) {
      // b.stroke(rgb, 5);
      // b.ellipse(branch.position.x, branch.position.y, size, size);
      if (branch.parent != null) {

        b.stroke(rgb);
        b.line(branch.parent.position.x, branch.parent.position.y, branch.position.x, branch.position.y);
        //b.point(branch.position.x, branch.position.y);
      }
    }
  }

  void debugBranch(PGraphics b, ArrayList<Branch> list, color rgb) {
    b.stroke(rgb);
    b.strokeWeight(2);
    b.strokeCap(ROUND);
    randomSeed(1000);
    ...
    int i=0;
    for (Branch branch : list) {
      float d = PVector.dist(branch.position, root.position);
      d = d/maxRadius;
      d = 1.0 - (d * 0.75 + 0.25);
      if (branch.parent != null) {
        float noise = noise(branch.position.x * 0.025, branch.position.y * 0.025);
        if (noise + random(0.25) > 0.45) {
          b.strokeWeight(noise * 3);
          b.line(branch.parent.position.x, branch.parent.position.y, branch.position.x, branch.position.y);
        }
        // b.vertex(branch.parent.position.x, branch.parent.position.y);
        //b.vertex(branch.position.x, branch.position.y);
      }
    }
    //b.endShape();
  }
