...
static class DayDatas {
  static public PVector getData(String month, String day, String weekday, String start, String slunch, String elunch, String end) {
    ...
    fend = (fend <= felunch && felunch != 0.0) ? 24.0 + fend : fend;
    
    float hourWorked = fslunch - fstart + fend - felunch;
    float baseHour = 12 - 9 + 18 - 13;
    float additionalHours = (hourWorked > 0.0 ) ? hourWorked - baseHour : 0.0;
    additionalHours = (additionalHours > 0.0) ? additionalHours : 0.0;
    additionalHours = (iwday > 5.0) ? hourWorked : additionalHours;
    println(iwday, "\t", iday[0]+"/"+iday[1]+"/"+iday[2], "\t", fstart, "\t", fslunch, "\t", felunch, "\t",fend, "\t", hourWorked, "\t", additionalHours, "\t", baseHour);
    data = new PVector(hourWorked, additionalHours);
    return data;
  }
